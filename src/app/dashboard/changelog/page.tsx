...
const CHANGELOG: LogEntry[] = [
  // ── 2026-03-25 ──
  {
    date: "2026-03-25",
    tag: "feat",
    title: "AI 대본 엔진 UX/UI 전면 개편 및 브라우저 캐시 버그 수정",
    details: [
      "자막 스튜디오와 100% 동일한 '현재 작업 저장' 및 '저장된 작업(세션)' 토글 UI 적용",
      "로컬 영상 반복 캡처 시 브라우저 캐시에 의해 이전 영상 이미지가 나오는 현상 해결 (Cache Buster 적용)",
      "로컬 영상의 경우 파일명을 기준으로 DB에 독립적으로 분리 저장 (덮어쓰기 로직 개선)",
    ],
  },
  {
    date: "2026-03-25",
    tag: "fix",
    title: "AI 대본 엔진 타임스탬프 맵핑(할루시네이션) 치명적 오류 해결",
    details: [
      "로컬 영상 업로드 시 영상 전체 길이를 파악하지 못해 대본이 무한정 길어지는 현상 해결 (길이 선행 계측 로직 추가)",
      "자막 스튜디오 연동 시, 자막의 원본 타임스탬프 데이터가 GPT 프롬프트 내에서 유실되는 버그 완벽 수정 ([00:15] 형태 맵핑)",
    ],
  },
  {
    date: "2026-03-25",
    tag: "fix",
    title: "로컬 영상 DB 저장 및 썸네일 모달 400 에러 해결",
    details: [
      "로컬 파일(MP4)은 videoId가 없어서 썸네일 클릭 시 발생하던 YouTube 플레이어 에러 방지 처리",
      "로컬 영상의 경우 프리뷰 모달 창에서 YouTube 대신 즉각 HTML5 <video> 플레이어로 전환하여 로드되도록 대응",
      "백엔드 DB가 빈 문자열(videoId='')을 필수값 누락으로 착각해 저장을 튕겨내던 API 검증 로직 완화",
    ],
  },
  {
    date: "2026-03-25",
